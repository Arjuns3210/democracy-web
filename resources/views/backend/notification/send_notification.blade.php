                            ...
                            <form id="sendNotification" method="post" action="notification/send">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4 col-lg-4 col-sm-4 mb-2">
                                        ...
                                        <input type="date" class="form-control required" id="date_time" name="date_time"><br/>
                                    </div>
                                    <div class="col-md-2 col-lg-2 col-sm-2 mb-2">
                                        ...
                                        <select class="form-control select2 required" id="trigger_time" name="trigger_time">
                                            @for($i=0;$i<=23;$i++)
                                                <option value="{{$i}}">{{($i <= 9 ) ? '0'.$i  : $i }} : 00</option>
                                            @endfor
                                        </select>
                                    </div>
                                    <div class="col-md-6 col-lg-6 col-sm-6 mb-2">
                                        <label>user<span class="text-danger">*</span></label>
                                        <select class="form-control select2 required" id="user_ids" name="user_ids[]" multiple>
        ...
        $('#user_ids').on('change', function() {
            ...
            if (selectedValues.length != 0){
                if (selectedValues.includes('all')) {
                    $('#user_ids option').prop('disabled', true);
                    $('#user_ids option[value="all"]').prop('disabled', false);
                } else {
                    $('#user_ids option').prop('disabled', false);
                    $('#user_ids option[value="all"]').prop('disabled', true);
                }
            }else{
                $('#user_ids option').prop('disabled', false);
            }
