...
class StaffController extends Controller
{
    ...
    public function store(CreateStaffRequest $request)
    {
        try {
            ...
            $input['is_head'] = $request->is_head=="on" ? '1': '0';
    ...
    public function changeStaffPassword(ChangeStaffPasswordRequest $request)
    {
        $msg_data = array();
        $admin = Admin::find($_GET['id']);

        if ($admin->password == md5($admin->email . $request->new_password)) {
            errorMessage('New Password cannot same as current password', $msg_data);
        }
