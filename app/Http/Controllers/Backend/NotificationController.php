...
class NotificationController extends Controller
{
    ...
    public function store(AddNotificationRequest $request)
    {
        try {
            DB::beginTransaction();
            $input = $request->all();
            $input['type'] = $input['type'] == "Home" ? 'HomeCollection' : $input['type'];
            if($input['type'] == "HomeCollection"){
                $input['selected_id'] ='0';
            }
            $notification = Notification::create($input);
            //store audio cover image
            if (! empty($input['image']) && $notification) {
                storeMedia($notification, $input['image'], Notification::NOTIFICATION_IMAGE,$notification->id);
            }
            DB::commit();
    ...
    {
        $data = [];
        if ($type == Notification::HOME){
            $data['home'] = [];
        }
        if ($type == Notification::QUESTION){
            $data['question'] = Question::where('status', '1')->get();
        }
        if ($type == Notification::CONTEST){
            $data['contest'] = Contest::where('status', '1')->get();
        }
    ...
    public function edit($id)
    {
        ...
        if ($type == Notification::QUESTION){
            $data['questions'] =  $selectedData['question'] ?? [];
        }

        if ($type == Notification::CONTEST){
            $data['contests'] =  $selectedData['contest'] ?? [];
        }
    ...
    {
        $data['notification'] = Notification::find($id);
        $data['image'] = $data['notification']->getMedia(Notification::NOTIFICATION_IMAGE)->first();
